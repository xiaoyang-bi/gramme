...
class Compose(object):
    ...
    def __call__(self, images, radar_povs, intrinsics):
        for t in self.transforms:
            images, radar_povs, intrinsics = t(images, radar_povs, intrinsics)
        return images, radar_povs, intrinsics
...
class Normalize(torch.nn.Module):
    ...
    def forward(self, tensors: List[Tensor], radar_povs, intrinsics) -> Tensor:
        """
        Args:
        ...
        return tensors, radar_povs, intrinsics
...
class RandomScaleCrop(torch.nn.Module):
    ...
    def forward(self, images, radar_povs, intrinsics):
        """
        Args:
            img (PIL Image or Tensor): Image to be flipped.
        Returns:
        ...
        radar_povs_cropped = [F.crop(radar_pov, offset_y, offset_x, in_h, in_w)
                          for radar_pov in radar_povs]

        for i in output_intrinsics:
            i[0, 2] -= offset_x
            i[1, 2] -= offset_y

        return cropped_images, torch.stack(radar_povs_cropped), output_intrinsics
...
class ColorJitter(torch.nn.Module):
    ...
    def forward(self, imgs, radar_povs, intrinsics):
        """
        Args:
        ...
        return imgs, radar_povs, intrinsics
